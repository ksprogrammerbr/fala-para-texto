document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!SpeechRecognition) {
    alert(
      "Reconhecimento de fala não é suportado neste navegador. Tente usar o Google Chrome."
    );
    return;
  }
  ...
  console.log("Iniciando aplicação...Correto");
  ...
  recognition.onend = () => {
    ...
    if (recognizing && !manualStop) {
      if (/Mobi|Android/i.test(navigator.userAgent)) {
        console.warn(
          "Detecção de dispositivo móvel: Não reiniciando automaticamente."
        );
        recognizing = false;
  ...
  function toggleSpeechRecognition() {
    if (recognizing) {
      ...
      if (
        ...
        console.error(
          "navigator.mediaDevices ou getUserMedia não disponíveis."
        );
        alert(
      ...
      navigator.mediaDevices
        ...
        .catch((err) => {
          alert(
            "Erro ao acessar o microfone. Verifique as permissões no navegador ou reinicie o dispositivo."
          );
